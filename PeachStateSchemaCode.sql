...
CREATE TABLE IF NOT EXISTS `mb_B6`.`Screening` (
  ...
  INDEX `fk_Screening_Film1_idx` (`Film_f.id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mb_B6`.`Reservation/Tickets` (
  ...
  INDEX `fk_Reservation/Tickets_Screening1_idx` (`Screening_screen.id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Reservation/Tickets_Screening1`
    FOREIGN KEY (`Screening_screen.id`)
    REFERENCES `mb_B6`.`Screening` (`screen.id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mb_B6`.`Screening Room` (
  ...
  `Screening_screen.id` INT NOT NULL,
  PRIMARY KEY (`Venue_ven.id`, `Screening_screen.id`),
  INDEX `fk_Screening_has_Venue_Venue1_idx` (`Venue_ven.id` ASC) VISIBLE,
  INDEX `fk_Screening Room_Screening1_idx` (`Screening_screen.id` ASC) VISIBLE,
  CONSTRAINT `fk_Screening_has_Venue_Venue1`
    FOREIGN KEY (`Venue_ven.id`)
    REFERENCES `mb_B6`.`Venue` (`ven.id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Screening Room_Screening1`
    FOREIGN KEY (`Screening_screen.id`)
    REFERENCES `mb_B6`.`Screening` (`screen.id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `mb_B6`.`Assignment` (
  ...
  PRIMARY KEY (`assignment.shift.id`, `assignment.staff.id`),
